...
$(() => {
    ...
    const cubeAnimation = setInterval(() => {
        changeCurrent();
        updateAction();
    }, 1250);

    function updateAction() {
        currentCube = cubes.eq(currentIndex);
        currentTech = technologies[currentIndex];
        changeTechName();
        changeDescription();

        cubes.each((i, element) => {
            const cube = $(element);
            const cubeIndex = cube.attr('id').split('-')[1] * 1;
            const faces = cube.children('div');
    ...
    dropdownBtn.click(() => {
        const actualClass = dropdownSymbol.attr('class').split(' ')[1];

        dropdownSymbol.toggleClass('fa-xmark fa-caret-down');

        if (actualClass === 'fa-xmark') {
            techDescription.removeClass('active');
        } else if (actualClass === 'fa-caret-down') {
            techDescription.addClass('active');
        }
    ...
    function changeTechName() {
        techName.text(currentTech.name).css('color', currentTech.defaultColor);
        dropdownBtn.css('color', currentTech.defaultColor);
    }

    function changeDescription() {
        techDescription.text(currentTech.description);
    }
});